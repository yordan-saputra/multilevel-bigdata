\section{Introduction}
    ...
    \begin{frame}
        ...
        \begin{itemize}
            \item High number of groups
            \item Large group sizes \footcites{clark2019}{speelman2018}
            \item Design matrix construction \footcite{wood2014}
        \end{itemize}
    \end{frame}
    ...
    \note{
        ...
        \begin{itemize}
            \item Large number of groups: Computational bottleneck from numerical integration over random effects for each group at every optimization step, leading to high computational costs
            \item Large group sizes: High-dimensional multivariate distributions create numerical issues with large covariance matrix inversion, even in linear models
            \item construction of the full design matrix $X$ leads to high computational costs. In GAM, the estimator $\hat{\beta} = (X^T X + \sum \lambda_j S_j)^{-1}X^T y$ is hard to compute when $X$ is large.
